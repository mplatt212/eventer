...
const AddEventModal = ({edit, setEdit, navigation}: IProps) => {
  const {colors} = useTheme();
  const [name, setName] = useState<string>('');
  const [location, setLocation] = useState<string>('');
  const [editDayOffset, setEditDayOffset] = useState<number>(-1);
  ...
  const onConfirm = useCallback(
    ({startDate, endDate}: IDates) => {
      if (edit) {
        const date_1 = startDate as Date;
        const date_2 = new Date(store.selectedEvent?.start_date as string);
        const dayDiff = getDayDiff(date_1, date_2);
        setEditDayOffset(dayDiff);
      }
    ...
    [setDatePickerOpen, setDateRange, setValue, edit],
  );
  ...
  const submitData = async (event: any) => {
    ...
    const url = `${process.env.REACT_APP_BASE_URL}/new_event/${userID}/${edit ? editDayOffset : 0}`;
    await fetch(
      url,
      options,
    )
