...
import {Title, Button, useTheme, ActivityIndicator, Portal} from 'react-native-paper';
...
const CreateAccountModal = () => {
  ...
  return (
    <Modal visible={store.authStore.createAccountModalOpen}>
      <Portal>
        <Modal visible={store.authStore.createUserLoading} style={{padding: 100}} transparent={true}>
          <View style={{backgroundColor: 'rgba(0, 0, 0, .5)', width: '100%', height: '100%'}}>
            <ActivityIndicator animating={store.authStore.createUserLoading} color={colors.accent} size='large' 
            style={{marginTop: 150}} />
          </View>
        </Modal>
      </Portal>
      <View
        ...
        <Button
          ...
          onPress={handleSubmit(data => handleCreateAccount(data))}>
          Create Account
        </Button>
        </ScrollView>
      </View>
